...
do
  # only run with the small critical section version with rtm
  ./test_variable_critical -p ${i} -c 10000 -o >> $ORIG_PWD/logs_${extend}_threadvar/rtm_tot_threadvar.txt
done


echo "#### RTM [1*16, 30*16] threads, Long critical less activity uneven \
./test_variable_critical -p tcount -c 1000 -r ####" >>\
 $ORIG_PWD/logs_${extend}_threadvar/rtm_tot_threadvar.txt

for i in {1..30}
do
  # only run with the small critical section version with rtm
  ./test_variable_critical -p ${i} -c 1000 -r >> $ORIG_PWD/logs_${extend}_threadvar/rtm_tot_threadvar.txt
done
...
echo "#### Spin *always* [1*16, 30*16] threads, Long critical less activity uneven \
./test_variable_critical -p tcount -c 1000 -r ####" >>\
 $ORIG_PWD/logs_${extend}_threadvar/spin_alw_tot_threadvar.txt
...
do
  # only run with the small critical section version with rtm
  ./test_variable_critical -p ${i} -c 1000 -r >> $ORIG_PWD/logs_${extend}_threadvar/spin_alw_tot_threadvar.txt
done
...
echo "#### Spin *yield* [1*16, 30*16] threads, Long critical less activity uneven\
./test_variable_critical -p tcount -c 1000 -r ####" >>\
 $ORIG_PWD/logs_${extend}_threadvar/spin_yield_tot_threadvar.txt
...
do
  # only run with the small critical section version with rtm
  ./test_variable_critical -p ${i} -c 1000 -r >> $ORIG_PWD/logs_${extend}_threadvar/spin_yield_tot_threadvar.txt
done
...
echo "#### Spin *exp* [1*16, 30*16] threads, Long critical less activity uneven \
./test_variable_critical -p tcount -c 1000 -r ####" >>\
 $ORIG_PWD/logs_${extend}_threadvar/spin_exp_tot_threadvar.txt
...
do
  # only run with the small critical section version with rtm
  ./test_variable_critical -p ${i} -c 1000 -r >> $ORIG_PWD/logs_${extend}_threadvar/spin_exp_tot_threadvar.txt
done
...
echo "#### qvol *always* [1*16, 30*16] threads, Long critical less activity uneven \
./test_variable_critical -p tcount -c 1000 -r ####" >>\
 $ORIG_PWD/logs_${extend}_threadvar/qvol_alw_tot_threadvar.txt
...
do
  # only run with the small critical section version with rtm
  ./test_variable_critical -p ${i} -c 1000 -r >> $ORIG_PWD/logs_${extend}_threadvar/qvol_alw_tot_threadvar.txt
done
...
echo "#### qvol *yield* [1*16, 30*16] threads, Long critical less activity uneven \
./test_variable_critical -p tcount -c 1000 -r ####" >>\
 $ORIG_PWD/logs_${extend}_threadvar/qvol_yield_tot_threadvar.txt
...
do
  # only run with the small critical section version with rtm
  ./test_variable_critical -p ${i} -c 1000 -r >> $ORIG_PWD/logs_${extend}_threadvar/qvol_yield_tot_threadvar.txt
done
...
echo "#### qvol *exp* [1*16, 30*16] threads, Long critical less activity uneven \
./test_variable_critical -p tcount -c 1000 -r ####" >>\
 $ORIG_PWD/logs_${extend}_threadvar/qvol_exp_tot_threadvar.txt
...
do
  # only run with the small critical section version with rtm
  ./test_variable_critical -p ${i} -c 1000 -r >> $ORIG_PWD/logs_${extend}_threadvar/qvol_exp_tot_threadvar.txt
done
...
echo "#### qvol *prop* [1*16, 30*16] threads, Long critical less activity uneven \
./test_variable_critical -p tcount -c 1000 -r ####" >>\
 $ORIG_PWD/logs_${extend}_threadvar/qvol_prop_tot_threadvar.txt
...
do
  # only run with the small critical section version with rtm
  ./test_variable_critical -p ${i} -c 1000 -r >> $ORIG_PWD/logs_${extend}_threadvar/qvol_prop_tot_threadvar.txt
done
...
echo "#### ompcrit [1*16, 30*16] threads, Long critical less activity uneven \
./test_variable_critical -p tcount -c 1000 -r ####" >>\
 $ORIG_PWD/logs_${extend}_threadvar/ompcrit_tot_threadvar.txt
...
do
  # only run with the small critical section version with rtm
  ./test_variable_critical -p ${i} -c 1000 -r >> $ORIG_PWD/logs_${extend}_threadvar/ompcrit_tot_threadvar.txt
done